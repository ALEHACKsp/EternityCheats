...
public:
	...
	bool create_overlay_window()
	{
		...
		{
			MessageBoxA(nullptr, "RegisterClassExA", "´íÎó", MB_OK | MB_ICONHAND);
			exit(-1);
		}
		...
		if (m_hwnd == NULL)
		{
			MessageBoxA(nullptr, "CreateWindowExA", "´íÎó", MB_OK | MB_ICONHAND);
			exit(-1);
		}
	...
	bool initialize()
	{
		...
		{
			MessageBoxA(nullptr, "Direct3DCreate9", "´íÎó", MB_OK | MB_ICONHAND);
			exit(-1);
		}
		...
		if (result != D3D_OK)
		{
			MessageBoxA(nullptr, "CreateDevice", "´íÎó", MB_OK | MB_ICONHAND);
			exit(-1);
		}
		...
		if (result != D3D_OK)
		{
			MessageBoxA(nullptr, "D3DXCreateLine", "´íÎó", MB_OK | MB_ICONHAND);
			exit(-1);
		}
		...
		if (result != D3D_OK)
		{
			MessageBoxA(nullptr, "D3DXCreateFontA", "´íÎó", MB_OK | MB_ICONHAND);
			exit(-1);
		}
		...
		if (m_IDirect3DDevice9)
		{
			...
			if (SUCCEEDED(m_IDirect3DDevice9->BeginScene()))
			{
				if (m_game == GetForegroundWindow())
				{
					...
					//render_text(50, 50, "Õâ¸ö¾ÍÊÇ²âÊÔ×ÖÌå", D3DCOLOR_ARGB(255, 255, 255, 0));
